---
title: 2.2. PHP
---
# <img src="../../img/php_azul.svg" width="40"> 2.2. PHP

<div style="text-align: center;"><img src="../../img/ut02/php_logo.png" style=" max-width: 30%;" /></div>

- Acrónimo de **P**ersonal **H**ome **P**age.
- Lenguaje de propósito general, aunque su fuerte es el desarollo web.
- Sintaxis similar a *C* / *Java*.
- El código se ejecuta en el servidor (en *Apache* mediante `mod_php`).
- El cliente recibe el resultado generado tras interpretar el código en el servidor.
- El código se almacena en archivo con extensión **`.php`**.

La versión actual (*a fecha de septiembre de 2026*) es la **8.5.10**. Además de numerosas nuevas funcionalidades que iremos viendo durante el curso, tiene más de dos veces mejor rendimiento que PHP5.

Su documentación es extensa y está traducida: [**https://www.php.net/manual/es/**](https://www.php.net/manual/es/){:target="_blank"}.

> **Versión de php**
> 
> Para comprobar tu versión de php puedes utilizar la `phpversion()` función:
> ```php
> <?php
> echo phpversion(); 
> ?>
> ```


## 1. Código embebido

Los bloques de código se escriben entre **`<?php`** y **`?>`**.

Las sentencias se separan mediante **` ; `**.

!!!examplephp "Ejemplo en php"
    === "Código"	
        ```php
        <!DOCTYPE html>
        <html lang="es">
        <head>
          <meta charset="UTF-8">
          <title>PHP fácil</title>
        </head>
        <body>
          <!-- Muestra una frase con HTML -->
          Hola mundo<br>
          <!-- Muestra una frase con PHP -->
          <?php echo "Es muy fácil programar en PHP."; ?>
        </body>
        </html>
        ```
    === "Resultado"
        <div style="text-align: center;"><figure><img src="../../img/ut02/ut02_prova1.png" alt="prova 01" style="zoom:100%;border: 2px solid #83abd4;" /><figcaption class="figure-caption-small">🖼️ Figura 2.1: Ejemplo código .php embebido con sentencia ";"</figcaption></figure></div>


> **Sólo etiquetas de apertura**
> 
> Si nuestro código **sólo va a contener código PHP** y nada de html, como por ejemplo, cuando codifiquemos **clases o interfaces**, sólo pondremos la etiqueta de apertura, para así indicar que es una archivo de php puro.



## 2. Generando código

Tenemos tres posibilidades a la hora de generar contenido en nuestros documentos PHP:

- `echo expresión;`
- `<?= expresión ?>`
- `print (expresión);`

Las que vamos a utilizar son:

- `echo` cuando lo hagamos dentro de un bloque de instrucciones y 
- `<?=` cuando sólo vayamos a mostrar el valor de una variable dentro de un fragmento HTML.

!!!examplephp "Ejemplo en php"
    === "Código"	
        ```php
        <!DOCTYPE html>
        <html lang="es">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Echo y print</title>
        </head>
        <body>
          <p><?php echo "Este texto se mostrará en la página web."; ?></p>
          <p><?= "Este texto se mostrará en la página web." ?></p><br>
          <p><?php print("Este texto se mostrará en la página web."); ?></p>
        </body>
        </html>
        ```
    === "Resultado"
        <div style="text-align: center;"><figure><img src="../../img/ut02/ut02_prova2.png" alt="prova 01" style="zoom:100%;border: 2px solid #83abd4;" /><figcaption class="figure-caption-small">🖼️ Figura 2.2: Ejemplo contenido con "echo" o "<?="</figcaption></figure></div>

## 3. Comentarios

Podemos utilizar comentarios de una línea o de bloque:

!!!examplephp "Ejemplo comentarios"
    === "Código"
        ```php
          <?php
          // Este es un comentario de una sola línea

          # Este es otro comentario de una sola línea
          
          /*
          Este es
          un comentario
          que ocupa
          varias líneas
          */ 
          ?>
        ```
    === "Resultado"
        <div style="text-align: center;"><figure><img src="../../img/ut02/ut02_prova3_com.png" alt="prova 01" style="zoom:100%;border: 2px solid #83abd4;" /><figcaption class="figure-caption-small">🖼️ Figura 2.3: Ejemplo inserción comentarios en .php</figcaption></figure></div>


> **Abrievatura en VS Code de comentarios**
> 
> - Comentario de una línea: 	**`Ctrl`** + **`/`**
> - Comentario de bloque (multilínea):	**`Shift`** + **`Alt`** + **`A`**

## 4. Errores

Si hay un error de ejecución, se produce un *Fatal Error*.

<div style="text-align: center;"><figure><img src="../../img/ut02/errores.png" alt="errores" style="zoom:100%;border: 2px solid #83abd4;" /><figcaption class="figure-caption-small">🖼️ Figura 2.4: Ejemplo de error de ejecución</figcaption></figure></div>

Desde PHP 5 se lanzan como una excepción. Más adelante veremos el uso de `try` / `catch`.

## 5. Variables

- No es necesario declararlas previamente.
- Comienzan por **`$`**, por ejemplo `$nombre`, seguido de un caracter en letra minúscula (**recomendación**) o guión bajo **`_`**. Luego ya se pueden poner números.
- Son *case sensitive*: `$var != $vAR`.
- No se declara su tipo, el tipado es dinámico. Se asigna en tiempo de ejecución dependiendo del valor asignado.
- Conveniente: hay que inicializarlas, si no dan error.

!!!examplephp "Ejemplo variables"
    === "Código"
        ```php
        <?php
            $nombre = "Marta";
            $nombreCompleto = "Marta López";
            $numero = 123;
            $numero2 = 456;
            $pi = 3.14;
            $suerte = true;
            $sinValor;
            $_1 = "el primero";
        

            echo $sinValor;
        ?>
        ```
    === "Resultado"
        <div style="text-align: center;"><figure><img src="../../img/ut02/ut02_prova4.png" alt="prova 01" style="zoom:100%;border: 2px solid #83abd4;" /><figcaption class="figure-caption-small">🖼️ Figura 2.5: Error por no inicializar una variable</figcaption></figure></div>


### 5.1. Tipos de datos
Aunque a priori no hay tipos de datos, internamente PHP trabaja con:
 	
- cuatro tipos escalares: *bool*, *int*, *float* y *string,*
- cuatro tipos compuestos: *array*, *object*, *callable* e *iterable*
- Existe un tipo especial para *null* (más información [**aquí**](http://php.net/manual/es/language.types.null.php){:target="_blank"}).

> **Cómo obtener el tipo de datos**
> 
> Para obtener el tipo de datos de una variable utiliza la función `var_dump()`
> ```php
> <?php
>   $x = 2;
>   var_dump($x);
> 
>   var_dump(8);
>   var_dump("Alba");
>   var_dump(3.14);
>   var_dump(true);
>   var_dump([1, "hola", null]);
>   var_dump(NULL);
> ?>
> ```

### 5.2. Alcance
En PHP, las variables se pueden declarar en cualquier parte del script.

El alcance de una variable es la parte del script donde se puede hacer referencia a la variable o utilizarla.

PHP tiene tres alcances de variables diferentes:

- local
- global
- estático

!!!examplephp "Ejemplo de alcances"
    === "Alcance global"
        Una variable declarada **fuera de** una función tiene un **ámbito global** y solo se puede acceder a ella fuera de una función:

        ```php
        <?php
        $x = 5; // global

        function miTest() {
          // usar x dentro de esta función generará un error
          echo "<p>Variable x dentro de función es: $x</p>";
        }
        miTest();

        echo "<p>Variable x fuera de función es: $x</p>";
        ?>
        ```

    === "Alcance local"
        Una variable declarada **dentro de** una función tiene un **ámbito local** y solo se puede acceder a ella dentro de esa función:

        ```php
        <?php
        function miTest() {
          $x = 5; // local
          echo "<p>Variable x dentro de función es: $x</p>";
        }
        miTest();

        // usar x fuera de la función generará un error
        echo "<p>Variable x fuera de función es: $x</p>";
        ?>
        ```

    === "Estática"
        ```php
        <?php

        function contador() {
            static $numero = 0;

            $numero++;

            echo $numero . "<br>";
        }

        contador();
        contador();
        contador();

        ?>
        El resultado sería:
        ```bash
        1
        2
        3
        ```

## 6. Constantes

Son variables cuyo valor no varían.

Existen dos posibilidades:

- `define(NOMBRE, valor);`
- `const NOMBRE;      // a partir de PHP 5.3`

!!!examplephp "Ejemplo constantes"
    === "Código"
        ```php
        <?php
              define("PI", 3.1416);
              const IVA = 0.21;
        
              echo PI, " ", IVA; // No se pone el símbolo dolar
        ?>
        ```
    === "Resultado"
        <div style="text-align: center;"><figure><img src="../../img/ut02/ut02_prova5.png" alt="prova 01" style="zoom:100%;border: 2px solid #83abd4;" /><figcaption class="figure-caption-small">🖼️ Figura 2.6: Ejemplo mostrar constantes</figcaption></figure></div>

- Se declaran siempre en MAYÚSCULAS.
- Hay un conjunto de constantes ya predefinidas, también conocidas como *magic constants*: [**https://www.php.net/manual/es/language.constants.predefined.php**](https://www.php.net/manual/es/language.constants.predefined.php){:target="_blank"}.



??? ies "Ejercicios a realizar"
	- Ejercicio 201
	- Ejercicio 202
