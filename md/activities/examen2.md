...
???questionlaravel "1.a: Enumera al menos **4 componentes de software** necesarios para desarrollar y ejecutar una aplicación web con Laravel en local (por ejemplo: intérprete, servidor, SGBD, herramientas…)"
...
???questionlaravel "1.b: Explica brevemente qué papel tiene **PHP** dentro de ese conjunto"
    El servidor web llama a PHP para ejecutar el código de la aplicación (controladores, modelos, vistas Blade compiladas, etc.) y generar la HTML/JSON que se envía al navegador.

---

### 2. Composer y ecosistema PHP (0,5 pt)

???questionlaravel "2.a: Explica con tus palabras qué es **Composer** y qué problema resuelve en proyectos PHP modernos"
...
???questionlaravel "2.b: ¿Cómo crearías un proyecto laravel?"
    ```bash
...
???questionlaravel "4.a: Explica la diferencia entre **una vista Blade** individual y **una plantilla (layout)** en Laravel"
...
???questionlaravel "4.b: ¿Qué ventajas ofrece Blade para **separar lógica y presentación**? Nombra al menos dos (por ejemplo, herencia de vistas, secciones, directivas, etc.)"
...
???questionlaravel "4.c: Indica cómo se pasaría desde un controlador una colección de objetos `$products` a una vista `products/index.blade.php` (menciona el retorno del controlador, no hace falta escribir la vista)"
    ```php
...
???questionlaravel "5.a: Explica brevemente la diferencia entre definir una ruta con una *función anónima* y definirla apuntando a un *método de controlador*"
...
???questionlaravel "5.b: Escribe la línea de ruta que registrarías en `routes/web.php` para gestionar un recurso `Course` mediante un controlador `CourseController` para mostrar un solo curso usando el sistema de recursos de Laravel"
    ```php
...
???questionlaravel "5.c:  Nombra *cuatro métodos* típicos que genera un controlador de recurso y explica en una frase la finalidad de cada uno"
...
???questionlaravel "6.a: ¿Qué es una *migración* en Laravel y qué ventaja aporta en un proyecto de equipo? "
...
???questionlaravel "6.b: ¿Crea una migración de creación de la tabla `courses`?"
...
???questionlaravel "6.c: ¿Cómo se ejecuta la migración anterior?"
    ```bash
    php artisan migrate
    ```

???questionlaravel "6.d: Explica qué representa un **modelo Eloquent** y cómo se relaciona con las tablas de la base de datos"
...
???questionlaravel "6.e: ¿Para qué sirven los **seeders**? Indica un ejemplo de cuándo es útil usarlos (en qué fase del desarrollo / pruebas)"
...
<div style="text-align: center;"><img src="../../img/others/examen2_imag1.png" alt="ut03" style="max-width: 40%;" /></div>


???questionlaravel "B2.1.b"
    ```php
    public function up()
    {
