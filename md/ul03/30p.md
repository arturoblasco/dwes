...
???teolaravel "FAQ (Preguntas Frecuentes sobre la práctica)"
	* **Error "SQLSTATE[42S01]: Base table or view already exists" al ejecutar migraciones**
	
		Verifica los siguientes pasos:
	
		1. Ejecuta `php artisan migrate:status` para ver qué migraciones ya se ejecutaron
		2. Si necesitas recrear las tablas, ejecuta `php artisan migrate:fresh` (elimina todas las tablas y las recrea)
	...
	* **Error "SQLSTATE[HY000] [1215] Cannot add foreign key constraint" en migraciones**
	...
	* **Error "Class 'App\Models\NombreModelo' not found" en controladores o seeders**
	...
	* **Error "SQLSTATE[23000]: Integrity constraint violation" al ejecutar seeders**
		...
		5. Ejecuta `php artisan migrate:fresh --seed` para limpiar y recrear todo
	* **Los modelos no devuelven datos o devuelven colecciones vacías**
	
		Verifica los siguientes pasos:
		
		1. Ejecutaste los seeders correctamente con `php artisan db:seed`
		2. Verifica que hay datos en la tabla con `php tinker` y `NombreModelo::count()`
		...
		5. Verifica la conexión a la base de datos con `php artisan migrate:status`
	* **Error "Call to a member function on null" al acceder a relaciones**
	...
	* **El problema N+1: muchas consultas SQL al listar productos con categorías/ofertas**
	...
	* **Error "Mass assignment" al crear registros con `Model::create()`**
	...
	* **Los seeders no insertan datos o fallan silenciosamente**
		...
		4. Ejecuta con `php artisan db:seed --class=NombreSeeder` para ver errores específicos
		5. Revisa los logs en `storage/logs/laravel.log` para ver errores detallados
	* **Error al usar `attach()` para tabla pivot: "Column not found"**
	...
	* **¿Cómo verifico si las relaciones funcionan correctamente?**
	
		Usa Tinker para probar:
		
		1. Abre Tinker: `php artisan tinker`
	...
	* **Error "Too few arguments to function" en métodos de relación**
	...
	* **Los timestamps (`created_at`, `updated_at`) no se actualizan automáticamente**
	...
	* **¿Cómo depuro qué consultas SQL está ejecutando Eloquent?**
	
		Usa estas técnicas:
		
		1. Laravel Telescope: `php artisan telescope:install` y revisa la pestaña Queries
...
```
<figcaption class="figure-caption-small">🖼️ Figura 8.7: Modelo E/R de la práctica guiada</figcaption>
...
#### 2. Iniciar Laravel Sail (para opción A de la práctica 1)

!!!tenencuenta "Opcional"
    Si has realizado la instalación de Laravel en Docker en la primera práctica guiada.
...
!!!tenencuenta "Importante"
    Las migraciones deben ejecutarse en orden. Por eso creamos primero `categories` y `offers`, y luego `products` que las referencia.
...
!!!tenencuenta "Recuerda la convención de Laravel"
    Las tablas pivot siguen el orden alfabético de los nombres de las tablas en singular: `product_user` (no `user_product`).

&nbsp;

!!!tenencuenta "Tabla pívot enriquecida"
...
!!!questionlaravel "Ejecuta el siguiente comando para crear el seeder"
    ...
    ???examplelaravel "Ejemplo de respuesta"
        ...
        ```bash
        artur@pc_classe:~/development/laravel/MyShop$ php artisan make:seeder OfferSeeder
...
!!!questionlaravel "Ejecuta el siguiente comando para crear el seeder"
    ...
    ???examplelaravel "Ejemplo de respuesta"
        ...
        ```bash
        artur@pc_classe:~/development/laravel/MyShop$ php artisan make:seeder ProductSeeder
...
!!!tenencuenta "Importante"
    Laravel maneja automáticamente las claves foráneas (`category_id`, `offer_id`) basándose en los IDs de los datos mock.
...
!!!tenencuenta "Importante"
    Los items del carrito deben crearse **después** de los usuarios y productos, ya que la tabla pivot referencia ambas tablas.

!!!questionlaravel "Ejecuta el siguiente comando para crear el seeder"

    ```bash
    php artisan make:seeder ProductUserSeeder
    ```

    ???examplelaravel "Ejemplo de respuesta"

        ```bash
        artur@pc_classe:~/development/laravel/MyShop$ php artisan make:seeder ProductUserSeeder
...
!!!questionlaravel "Ejecuta el siguiente comando para ejecutar todos los seeders"
    ...
    ???examplelaravel "Ejemplo de respuesta"
        ...
        ```bash
        artur@pc_classe:~/development/laravel/MyShop$ php artisan db:seed
